...
import {
  ...
  MessageSquare, ScrollText, HelpCircle, Sparkles, X,
} from 'lucide-react'
...
export default function Sidebar({ drawerOpen = false, onClose }: SidebarProps) {
  ...
  return (
    <aside
      className={`
        bg-slate-900 flex flex-col h-screen border-r border-slate-800/60
        w-[88%] max-w-[300px] lg:w-72 lg:flex-shrink-0
    ...
    >
      ...
      <nav className="flex-1 px-4 py-5 overflow-y-auto">
        <div className="mb-6">
          <p className="text-slate-500 text-[11px] font-bold uppercase tracking-[0.12em] px-3.5 mb-3">Main Menu</p>
          <div className="space-y-1">{mainNav.map(renderItem)}</div>
        </div>
        <div className="mb-6">
          <p className="text-slate-500 text-[11px] font-bold uppercase tracking-[0.12em] px-3.5 mb-3">Management</p>
          <div className="space-y-1">{managementNav.map(renderItem)}</div>
        </div>
        <div className="mb-6">
          <p className="text-slate-500 text-[11px] font-bold uppercase tracking-[0.12em] px-3.5 mb-3">System</p>
          <div className="space-y-1">{systemNav.map(renderItem)}</div>
        </div>

        {/* Promo card — hidden on small screens to save space */}
        <div className="hidden sm:block mt-8 p-4 rounded-xl bg-gradient-to-br from-indigo-600/25 via-indigo-700/15 to-violet-700/25 border border-indigo-500/20 relative overflow-hidden">
